...
const syncHeroVisual = () => {
  ...
  if (!heroStage || !heroVisual || prefersReducedMotion) {
    if (heroStage) {
      ...
      heroStage.style.setProperty("--hero-mobile-lift", "0px");
      heroStage.style.setProperty("--hero-mobile-scale", "1.01");
    }
    return;
  }
  ...
  heroStage.style.setProperty("--hero-mobile-lift", `${(-12 * mobileLiftProgress).toFixed(2)}px`);
  heroStage.style.setProperty("--hero-mobile-scale", (1.01 + mobileLiftProgress * 0.02).toFixed(3));
};
